...
* 重复复制已在历史列表第一位的内容时，弹出粉红色「已复制」气泡：保持画圆动画，圆内图标为向右箭头
...
* 修复分辨率 / 缩放 / 显示器变化后弹窗显示不完整的问题：检测到显示器配置变化时自动重启应用并重置窗口几何存档，按新分辨率重新初始化布局
...
* 提取到的 QQ 号先经 Qzone 公开接口核验存在，确认不存在的不显示；接口不可达时降级保留
* 文本内实体（链接 / 邮箱 / 手机号 / QQ）超过 4 个时默认折叠，点击「+N」展开、「收起」恢复
...
* 新增复制成功气泡窗与窗口生命周期管理
* 强化剪贴板读写链路：写回重试与缩略图同步生成

### 🐛 问题修复

* 扩展邮箱本地部分的正则匹配，包含 . _ + - % (#1403)
* 备份合并导入时保留敏感标记 (#1404)
* 短关键词搜索时回退到 LIKE 查询 (#1407)
* 识别带引号 JSON 风格键名的敏感信息 (#1408)
* 剪贴板被短暂占用时写回自动重试
* 同步生成图片缩略图，避免渲染原图导致卡顿
* 复制与最近一条相同的内容时不再弹出气泡
